...
namespace corn {
    FontManager::FontManager() : fontManagerImpl_(new FontManagerImpl()) {}

    FontManager::~FontManager() {
        this->unloadAllFontFamilies();
        delete this->fontManagerImpl_;
    }
    ...
    bool FontManager::loadFontFamilyFromPath(const std::string& name, const std::filesystem::path& path) {
        ...
        // Create the font family
        FontFamily* fontFamily = FontFamily::createFromPath(path, this->fontManagerImpl_->fontCollectionLoader);
        if (!fontFamily) {
            return false;
        }
    ...
    bool FontManager::loadFontFaceFromPath(const std::string& name, const std::filesystem::path& path, const FontVariant& fontVariant) {
        ...
        // Add the font face
        return fontFamily->addFontFace(path, fontVariant, this->fontManagerImpl_->fontCollectionLoader);
    }
