...
corn::Entity* createCamera(corn::EntityManager& entityManager) {
    ...
    camera->addComponent<corn::CTransform2D>(corn::Vec2f((float)WIDTH / 2, (float)HEIGHT / 2));
    auto* ccamera = camera->addComponent<corn::CCamera>(
            corn::Vec2f::ZERO(), corn::Color::rgb(60, 179, 113));
    ccamera->setViewport(
...
corn::Entity* createBird(corn::EntityManager& entityManager) {
    ...
    auto transform = bird->addComponent<corn::CTransform2D>(corn::Vec2f(400, 300));
...
corn::Entity* createWall(corn::EntityManager& entityManager, float x) {
    ...
    top->addComponent<corn::CTransform2D>(corn::Vec2f::ZERO());
    top->addComponent<corn::CBBox>(corn::Vec2f::ZERO(), corn::Vec2f(WALL_THICKNESS, topWallSize));
    ...
    bottom->addComponent<corn::CTransform2D>(corn::Vec2f(0, topWallSize + HOLE_SIZE));
    bottom->addComponent<corn::CBBox>(corn::Vec2f::ZERO(), corn::Vec2f(WALL_THICKNESS, bottomWallSize));
...
void createCeilAndFloor(corn::EntityManager& entityManager) {
    ...
    // Components of ceil
    auto ceilTransform = ceil->addComponent<corn::CTransform2D>(corn::Vec2f::ZERO());
    ceilTransform->setZOrder(1);
    ceil->addComponent<corn::CBBox>(corn::Vec2f::ZERO(), corn::Vec2f(WIDTH, CEIL_THICKNESS));
    ...
    auto floorTransform = floor->addComponent<corn::CTransform2D>(
            corn::Vec2f(0, HEIGHT - CEIL_THICKNESS));
    floorTransform->setZOrder(1);
    floor->addComponent<corn::CBBox>(corn::Vec2f::ZERO(), corn::Vec2f(WIDTH, CEIL_THICKNESS));
